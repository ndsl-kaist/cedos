...
public class XMLRefresh extends DefaultHandler {
	...
	public XMLRefresh(RSSFeed feed) {
		super();
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if (localName.equalsIgnoreCase(TAG_ITEM)) {
			...
			/* NOTE: since this is podcast application, we do not care about
			 * the items without enclosure field */
			if (currentitem.enclosure_url.equals("")) {
				return;
				/*
				for (int i = 0; i < currentfeed.items.size(); i++) {
					if (currentfeed.items.get(i).pubDate == currentitem.pubDate)
						return;
				}
				*/
			}
				...
				for (int i = 0; i < currentfeed.items.size(); i++) {
					if (currentfeed.items.get(i).enclosure_url.equals(currentitem.enclosure_url)) {
						return;
					}
				}
			}
			...
			currentfeed.items.add(index++, currentitem);
			isParsing = false;
		}
