...
static ngx_int_t ngx_http_variable_server_port(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_flow_id(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_host_id(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
static ngx_int_t ngx_http_variable_deadline(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_http_variable_t  ngx_http_core_variables[] = {
    ...
    { ngx_string("flow_id"), NULL, ngx_http_variable_flow_id, 0, 0, 0 },

    { ngx_string("host_id"), NULL, ngx_http_variable_host_id, 0, 0, 0 },

    { ngx_string("deadline"), NULL, ngx_http_variable_deadline, 0, 0, 0 },
...
static ngx_int_t
ngx_http_variable_flow_id(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    uint32_t            flow_id;
    v->data = ngx_pnalloc(r->pool, NGX_INT32_LEN); //sizeof("4294967275") - 1);
    if (v->data == NULL) {
        return NGX_ERROR;
    }
	...
	flow_id = dtp_getflowid(r->connection->fd);
	v->len = ngx_sprintf(v->data, "%ui", flow_id) - v->data;

    return NGX_OK;
}

#define NGX_HOST_ID_STRLEN 20
static ngx_int_t
ngx_http_variable_host_id(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
	u_char* host_id;
	if ((host_id = dtp_gethostid(r->connection->fd)) == NULL) {
		return NGX_ERROR;
	}

    v->data = ngx_pnalloc(r->pool, NGX_HOST_ID_STRLEN);
    if (v->data == NULL) {
        return NGX_ERROR;
    }
	v->len = NGX_HOST_ID_STRLEN;

    ngx_memcpy(v->data, host_id, v->len);
...
static ngx_int_t
ngx_http_variable_deadline(ngx_http_request_t *r,
    ngx_http_variable_value_t *v, uintptr_t data)
{
    uint32_t            deadline;
	uint32_t            len;
	...
	if (dtp_getsockopt(r->connection->fd, 0, DTP_SO_DEADLINE,
					   &deadline, &len) == -1) {
		return NGX_ERROR;
	}
	v->len = ngx_sprintf(v->data, "%ui", deadline) - v->data;
