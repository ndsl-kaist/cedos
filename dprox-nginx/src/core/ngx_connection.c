    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
			...
			*/
            ...
            if (dtp_bind(s, ls[i].sockaddr, ls[i].socklen) == -1) {
                ...
                ngx_log_error(NGX_LOG_EMERG, log, err,
                              "dtp_bind() to %V failed", &ls[i].addr_text);
            ...
            if (dtp_listen(s, ls[i].backlog) == -1) {
                ngx_log_error(NGX_LOG_EMERG, log, ngx_socket_errno,
                              "dtp_listen() to %V, backlog %d failed",
...
ngx_configure_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ls[i].rcvbuf != -1) {
			if (dtp_isdtpsocket(ls[i].fd)) {
				if (setsockopt(ls[i].fd, SOL_SOCKET, DTP_SO_RCVBUF,
        ...
        if (ls[i].sndbuf != -1) {
			if (dtp_isdtpsocket(ls[i].fd)) {
				if (dtp_setsockopt(ls[i].fd, SOL_SOCKET, DTP_SO_SNDBUF,
        ...
        if (ls[i].listen) {
            ...
            if (dtp_listen(ls[i].fd, ls[i].backlog) == -1) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
                              "dtp_listen(2) to %V, backlog %d failed, ignored",
                              &ls[i].addr_text, ls[i].backlog);
            }
        }
