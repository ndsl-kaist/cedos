...
ngx_linux_sendfile_chain(ngx_connection_t *c, ngx_chain_t *in, off_t limit)
{
    ...
    for ( ;; ) {
        ...
        if (file) {
			...
			if (dtp_isdtpsocket(c->fd)) {
				/* dtp_sendfile() */
				rc = dtp_sendfile(c->fd, file->file->fd, &offset, file_size);
			}
			else {
			...
			if (dtp_isdtpsocket(c->fd)){
				...
				for (k = 0; k < (int)header.nelts; k++) {

					len = (((struct iovec*)header.elts)[k]).iov_len;

					res = dtp_write(c->fd, (void*)((char*)(((struct iovec*)header.elts)[k]).iov_base), len);
            ...
            if (rc == -1) {
                ...
                default:
                    wev->error = 1;
                    ngx_connection_error(c, err, "writev() failed (in linux_sendfile_chain.c)");
                    return NGX_CHAIN_ERROR;
                }
