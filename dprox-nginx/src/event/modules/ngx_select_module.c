...
static fd_set_s         master_read_fd_set;
static fd_set_s         master_write_fd_set;
static fd_set_s         work_read_fd_set;
static fd_set_s         work_write_fd_set;
...
ngx_select_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    if (event_index == NULL) {
        FD_ZERO_S(&master_read_fd_set);
        FD_ZERO_S(&master_write_fd_set);
        nevents = 0;
    }
...
ngx_select_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (event == NGX_READ_EVENT) {
        FD_SET_S(c->fd, &master_read_fd_set);

    } else if (event == NGX_WRITE_EVENT) {
        FD_SET_S(c->fd, &master_write_fd_set);
...
ngx_select_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (event == NGX_READ_EVENT) {
        FD_CLR_S(c->fd, &master_read_fd_set);

    } else if (event == NGX_WRITE_EVENT) {
        FD_CLR_S(c->fd, &master_write_fd_set);
    ...
    ngx_uint_t flags)
{
    ...
    ready = dtp_select(max_fd + 1, (fd_set*)&work_read_fd_set, (fd_set*)&work_write_fd_set, NULL, tp);
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (ev->write) {
            if (FD_ISSET_S(c->fd, &work_write_fd_set)) {
                found = 1;
                ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                               "select write %d", c->fd);
            ...
            if (FD_ISSET_S(c->fd, &work_read_fd_set)) {
                found = 1;
                ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                               "select read %d", c->fd);
            }
    ...
    if (ready != nready) {
		fprintf(stderr, "ready = %d, nready = %d\n", ready, nready);
...
ngx_select_repair_fd_sets(ngx_cycle_t *cycle)
{
    ...
    for (s = 0; s <= max_fd; s++) {
        ...
        if (FD_ISSET_S(s, &master_read_fd_set) == 0) {
            continue;
        }
        ...
        if (getsockopt(s, SOL_SOCKET, SO_TYPE, &n, &len) == -1) {
            ...
            FD_CLR_S(s, &master_read_fd_set);
        }
    }

    for (s = 0; s <= max_fd; s++) {

        if (FD_ISSET_S(s, &master_write_fd_set) == 0) {
            continue;
        }
        ...
        if (getsockopt(s, SOL_SOCKET, SO_TYPE, &n, &len) == -1) {
            ...
            FD_CLR_S(s, &master_write_fd_set);
        }
    }
...
ngx_select_init_conf(ngx_cycle_t *cycle, void *conf)
{
	...
	printf("[ngx_select_module.c:411] start ngx_select_init_conf()\n");
    ...
    if (cycle->connection_n > FD_SETSIZE_S) {
        ngx_log_error(NGX_LOG_EMERG, cycle->log, 0,
                      "the maximum number of files "
                      "supported by select() is %ud", FD_SETSIZE_S);
